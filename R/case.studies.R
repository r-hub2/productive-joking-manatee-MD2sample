...
#' @param n =500, sample size.
#' @param nx =n, sample size of x data set.
#' @param ny =n, sample size of y data set.
#' @param nbins =-1,number of bins if discrete data is wanted.
#' @param Ranges =matrix(c(-Inf, Inf, -Inf, Inf),2,2) a 2x2 matrix with lower and upper bounds
#' @param ReturnCaseNames =FALSE should only list of names be returned?
#' @keywords internal
#' @return a list of functions
#' @export
case.studies = function(which, n=200, nx=n, ny=n, nbins=-1, 
      Ranges =matrix(c(-Inf, Inf, -Inf, Inf),2,2), ReturnCaseNames=FALSE) {
  cases=c("NormalD2", "tD2", "UniformMixtureD2", "FrankD2", "ClaytonD2", 
          "GumbelD2", "GalambosD2", "HuslerReissD2", "ClaytonGumbelD2", "UniformFrankD2",  
          "ParetoSimplexD2", "KhoudrajiClaytonD2",  "NormalUniformD2", "JoeD2", "DalitzCleoD2", 
          "DalitzPDGD2", "DalitzBabarD2",  
          "NormalShiftM", "NormalStretchM", "UniformRotateM", "UniformBetaM", "TruncExponentialM", 
          "DalitzCleoM", "DalitzPDGM",  "DalitzBabarM", "FrankExponentialM","FrankLinearM",
          "FrankNormalM", "ClaytonExponentialM", "ClaytonLinearM", "ClaytonNormalM", "GalambosExponentialM",
  ...
  uniforms=function()  {
     x=matrix(stats::runif(2*nx), nx,2)
     y=matrix(stats::runif(2*ny), nx,2)
     out=list(x=x, y=y)
     if(nbins[1]>0) out=MD2sample::bincounterR(out, nbins=nbins, Ranges=Ranges)
     out
  }   
# Case Studies for D=2  
  if(which=="NormalD2") {
    return(list(
      f=function(a=0) {
       S=diag(2) 
  ...
  if(which=="UniformMixtureD2") {
     return(list( 
      f=function(alpha=0, d=2) {
        ...
        z[z[,2]<0, 2]=0
        z[z[,2]>1, 2]=1
        y=rbind(matrix(stats::runif(d*(ny-m)), ny-m, d), z)
        out=list(x=x, y=y)
        if(nbins[1]>0) out=MD2sample::bincounterR(out, nbins=nbins, Ranges=Ranges)
        out
      },
  ...
  if(which=="ClaytonGumbelD2") {
    return(list(
      f=function(a=1/2) {
        cc <- copula::claytonCopula(copula::iTau(copula::claytonCopula(), tau = 0.95)) # the first component
        gc <- copula::gumbelCopula(copula::iTau(copula::gumbelCopula(),   tau = 0.15)) # the second component
        wts1 <- c(1/2, 1/2) # the corresponding weights
        mcg1 <- copula::mixCopula(list(cc, gc), w = wts1)
        x <- copula::rCopula(nx, copula = mcg1)
        wts2 <- c(a, 1-a) # the corresponding weights
  ...
  if(which=="UniformFrankD2") {
    return(list(
      f=function(a=1/2) {
        un <- copula::indepCopula() # the first component
        fc <- copula::frankCopula(20, 2) # the second component
        wts <- c(1/2, 1/2) # the corresponding weights
        mcg <- copula::mixCopula(list(un, fc), w = wts)
        x <- copula::rCopula(nx, copula = mcg)
        wts <- c(a, 1-a) # the corresponding weights
        mcg <- copula::mixCopula(list(un, fc), w = wts)
  ...
  if(which=="ParetoSimplexD2") {  
    return(list(
      f=function(th=1, d=2) {
        incBeta <- function(x, a, b) stats::pbeta(x, a, b) * beta(a, b) 
  ...
  if(which=="KhoudrajiClaytonD2") {  
    return(list(
      f=function(a) {
        kcx <- copula::khoudrajiCopula(copula2 = copula::claytonCopula(6), shapes = c(0.2, 0.95))
        kcy <- copula::khoudrajiCopula(copula2 = copula::claytonCopula(6), shapes = c(a, 0.95))
        out=list(x=copula::rCopula(nx, copula = kcx), y=copula::rCopula(ny, copula = kcy))
        if(nbins[1]>0) out=MD2sample::bincounterR(out, nbins=nbins, Ranges=Ranges)
        out
      },
   ...
   if(which=="NormalUniformD2") {
    return(list(
      f=function(a=1/2) {      
        nc1 <- copula::indepCopula() # the first component
        nc2 <- copula::normalCopula(copula::iTau(copula::normalCopula(), tau = 0.5)) # the second component
   ...
   if(startsWith(which, "Dalitz")) {
     ...
     experiment=1
     if(startsWith(which, "DalitzPDG")) experiment=2
     if(startsWith(which, "DalitzBabar")) experiment=3
     return(list(
      f=function(a=0) {
...
# Cases with D=5  
  ...
  if(which=="FrankD5") {
    return(list(
      f=function(theta=0, d=5) {
        x=matrix(stats::runif(nx*d),ncol=d)
        if(theta==0) return(list(x=x,y=matrix(stats::runif(ny*d),ncol=d)))
        cop = copula::frankCopula(theta, d)
  ...
  if(which=="GumbelD5") {
    return(list(
      f=function(theta=1, d=5) {
        x=matrix(stats::runif(nx*d),ncol=d)
        if(theta==1) return(list(x=x,y=matrix(stats::runif(ny*d),ncol=d)))
        cop = copula::gumbelCopula(theta, d)
        list(x=x, y=copula::rCopula(ny, cop))
      },
      param_alt=c(1,1.25)
  ...
  if(which=="JoeD5") {
    return(list(
      f=function(a=1) { 
        ...
        list(x=copula::rCopula(nx, copula = nc1), 
             y=copula::rCopula(ny, copula = nc2))
      },
      param_alt=c(1, 2)
    ))  
  ...
  if(which=="UniformRotateM") {
    return(list(
      f=function(alpha=0) {
        ...
        y=matrix(stats::runif(2*ny), ncol=2)%*%matrix(c(cos(alpha), sin(alpha), -sin(alpha), cos(alpha)), 2, 2)
        out=list(x=x, y=y)
        if(nbins[1]>0) out=MD2sample::bincounterR(out, nbins=nbins, Ranges=Ranges)
        out  
      },
  ...
  if(which=="FrankExponentialM") {
    return(list(
      f = function(lambda=1) {
        ...
        out=list(x=x, y=y)
        out=MD2sample::change.marginals(out, "Exponential", lambda)
        if(nbins[1]>0) out=MD2sample::bincounterR(out, nbins=nbins, Ranges=Ranges)
        out
      },
      nbins=c(5,5),param_alt=c(1, 1.7)
